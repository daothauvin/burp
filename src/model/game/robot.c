...
void update_pos_robot(robot *rob)
{
    ...
    double x2 = rob->pos.x + (rob->speed * cos(degree_to_radians(rob->angle)));
    double y2 = rob->pos.y + (rob->speed * sin(degree_to_radians(rob->angle)));
...
void inflict_damage_from_missile(robot *rob, point p)
{
    ...
    if (d < low_range)
        rob->health_points = 
            rob->health_points - damage_explosion_low_range > 0 ? 
                rob->health_points - damage_explosion_low_range: 0;
    else if (d < middle_range)
        rob->health_points = 
            rob->health_points - damage_explosion_middle_range > 0 ? 
                rob->health_points - damage_explosion_middle_range: 0;
    else if (d < high_range)
        rob->health_points = 
            rob->health_points - damage_explosion_high_range > 0 ? 
                rob->health_points - damage_explosion_high_range: 0;
}
void inflict_damage_from_collision(robot *rob1, robot *rob2)
{
    float zero = 0;
    rob1->health_points = 
        rob1->health_points - collision_damage > 0 ? 
            rob1->health_points - collision_damage : 0;
    rob2->health_points = 
        rob2->health_points - collision_damage > 0 ? 
            rob2->health_points - collision_damage : 0;
    rob1->speed = zero;
    rob2->speed = zero;
}
